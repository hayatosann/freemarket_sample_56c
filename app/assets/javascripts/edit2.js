$(function(){
  ...
  var images = [];
  var update__img_view = $('.update__img-view');
  var update_images = [];
  var inputs  =[];
  var input_area = $('.input_area');
  update_images.push(update__img_view);
  $('.image-upload-box').css({
    'width': `calc(100% - (135px * ${(images.length)+(update__img_view.length)})`
  })
  $(".dropzone-area~.dropzone-area").css({
    'display': `none`
  })
  $(".dropzone-container~.dropzone-container").css({
    'display': `none`
  })
  ...
  $(document).on('change', 'input[type= "file"].upload-image',function(event) {
    ...
    images.push(img);
    if((images.length)+(update__img_view.length) >= 5) {
      ...
      $.each(images, function(index, image) {
        ...
        $('.image-upload-box2').css({
          'width': `calc(100% - (135px * ${(images.length)+(update__img_view.length)-5}))`,
        })
      })
      if((images.length)+(update__img_view.length) == 9) {
      ...
      if((images.length)+(update__img_view.length) == 4) {
        dropzone.find('p').replaceWith('<i class="fa fa-camera"></i>')
      }
    if((images.length)+(update__img_view.length) == 10) {
      $('.image-upload-box2').css({
        'display': 'none'
      })
      return;
  ...
  $(document).on('click', '.update__delete-btn', function() {
    var target_image_edit = $(this).parent().parent();
    target_image_edit.remove()
    if ((images.length)+(update__img_view.length) >= 5) {
      $('.image-upload-box2').css({
        'display': 'block'
      })
      ...
      if((images.length)+(update__img_view.length) == 8) {
        $('.image-upload-box2').find('i').replaceWith('<p>ココをクリックしてください</p>')
      }
    } else {
      $('.image-upload-box').css({
    ...
    if((images.length)+(update__img_view.length) == 3) {
      $('.image-upload-box').find('i').replaceWith('<p>ココをクリックしてください</p>')
    }
  });
});